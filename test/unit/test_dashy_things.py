...
def test_valid_bluemn_address():
    from bluemnlib import is_valid_bluemn_address

    main = valid_bluemn_address()
    test = valid_bluemn_address('testnet')

    assert is_valid_bluemn_address(main) is True
    assert is_valid_bluemn_address(main, 'mainnet') is True
    assert is_valid_bluemn_address(main, 'testnet') is False

    assert is_valid_bluemn_address(test) is False
    assert is_valid_bluemn_address(test, 'mainnet') is False
    assert is_valid_bluemn_address(test, 'testnet') is True


def test_invalid_bluemn_address():
    from bluemnlib import is_valid_bluemn_address

    main = invalid_bluemn_address()
    test = invalid_bluemn_address('testnet')

    assert is_valid_bluemn_address(main) is False
    assert is_valid_bluemn_address(main, 'mainnet') is False
    assert is_valid_bluemn_address(main, 'testnet') is False

    assert is_valid_bluemn_address(test) is False
    assert is_valid_bluemn_address(test, 'mainnet') is False
    assert is_valid_bluemn_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from bluemnlib import elect_mn
...
def test_parse_masternode_status_vin():
    from bluemnlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % bluemnlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(bluemnlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(bluemnlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(bluemnlib.blocks_to_seconds(16616)) == 2612035
