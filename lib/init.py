...
def has_bluemn_conf():
    ...
    valid_bluemn_conf = False

    # ensure bluemn_conf exists & readable
    #
    # if not, print a message stating that Bluemn Core must be installed and
    # configured, including JSONRPC access in bluemn.conf
    try:
        f = io.open(config.bluemn_conf)
        valid_bluemn_conf = True
    except IOError as e:
        print(e)

    return valid_bluemn_conf
...
def main():
    ...
    if not has_bluemn_conf():
        print("BluemnCore must be installed and configured, including JSONRPC access in bluemn.conf")
        sys.exit(1)
