...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, bluemnd):
        golist = bluemnd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_bluemnd(bluemnd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_bluemnd(self, bluemnd, rec):
        import decimal
        import bluemnlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from bluemnd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from bluemnd, with every run
        ...
        # get/create, then sync payment amounts, etc. from bluemnd - Bluemnd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(bluemnd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from bluemnd! %s" % e)
            govobj.vote_delete(bluemnd)
            return (govobj, None)
    ...
    def vote_delete(self, bluemnd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(bluemnd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, bluemnd, signal, outcome):
        import bluemnlib
        ...
        output = bluemnd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = bluemnlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(bluemnd, signal)

    def sync_network_vote(self, bluemnd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = bluemnd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = BLUEMND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 bluemn addr, non-multisig
            if not bluemnlib.is_valid_bluemn_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Bluemn address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Bluemn Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (bluemnlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = BLUEMND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not bluemnlib.is_valid_bluemn_address(addr, config.network):
    ...
    def hash(self):
        import bluemnlib
        return bluemnlib.hashit(self.serialise())
