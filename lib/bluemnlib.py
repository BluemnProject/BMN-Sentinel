...
def is_valid_bluemn_address(address, network='mainnet'):
    ...
    bluemn_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Bluemn address
        return False

    if (address_version != bluemn_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('bluemn.conf')
