...
sentinel_cfg = BluemnConfig.tokenize(sentinel_config_file)
sentinel_version = "1.3.0"
min_bluemnd_proto_version_with_sentinel_ping = 70207


def get_bluemn_conf():
    if sys.platform == 'win32':
        bluemn_conf = os.path.join(os.getenv('APPDATA'), "BluemnCore/bluemn.conf")
    else:
        home = os.environ.get('HOME')

        bluemn_conf = os.path.join(home, ".bluemncore/bluemn.conf")
        if sys.platform == 'darwin':
            bluemn_conf = os.path.join(home, "Library/Application Support/BluemnCore/bluemn.conf")

    bluemn_conf = sentinel_cfg.get('bluemn_conf', bluemn_conf)
