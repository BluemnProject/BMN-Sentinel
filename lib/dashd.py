...
class BluemnDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some bluemnd methods
    ...
    @classmethod
    def from_bluemn_conf(self, bluemn_dot_conf):
        from bluemn_config import BluemnConfig
        config_text = BluemnConfig.slurp_config_file(bluemn_dot_conf)
        creds = BluemnConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = bluemnlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = bluemnlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = bluemnlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_bluemnd_proto_version_with_sentinel_ping)
